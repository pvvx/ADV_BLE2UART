...
int app_le_adv_report_event_handle(u8 *p)
{
	...
	s8 rssi = pa->data[pa->len];
		...
		if(clock_time_exceed(tick_adv_rpt, 5000000)){
			tlkapi_send_string_data(APP_CONTR_EVENT_LOG_EN, "[APP][EVT] Adv report", pa->mac, 6);
...
int app_le_connection_complete_event_handle(u8 *p)
{
	...
	if(pConnEvt->status == BLE_SUCCESS){
		...
		tlkapi_send_string_data(APP_CONTR_EVENT_LOG_EN, "[APP][EVT] Connection complete event", &pConnEvt->connHandle, sizeof(hci_le_connectionCompleteEvt_t) - 2);
...
int 	app_disconnect_event_handle(u8 *p)
{
	...
	tlkapi_send_string_data(APP_CONTR_EVENT_LOG_EN, "[APP][EVT] disconnect event", &pCon->connHandle, 3);
...
int app_le_connection_update_complete_event_handle(u8 *p)
{
	...
	tlkapi_send_string_data(APP_CONTR_EVENT_LOG_EN, "[APP][EVT] Connection Update Event", &pUpt->connHandle, 8);
...
int app_host_event_callback(u32 h, u8 *para, int n)
{
	...
	switch(event)
	{
		...
		case GAP_EVT_SMP_PAIRING_FAIL:
		{
				...
				if( dev_char_get_conn_role_by_connhandle(p->connHandle) == ACL_ROLE_CENTRAL){  //master connection
					if(central_smp_pending == p->connHandle){
						central_smp_pending = 0;
						tlkapi_send_string_data(APP_SMP_LOG_EN, "[APP][SMP] paring fail", &p->connHandle, sizeof(gap_smp_pairingFailEvt_t));
					}
				}
			#endif
		}
		...
		case GAP_EVT_SMP_CONN_ENCRYPTION_DONE:
		{
			...
			tlkapi_send_string_data(APP_SMP_LOG_EN, "[APP][SMP] Connection encryption done event", &p->connHandle, sizeof(gap_smp_connEncDoneEvt_t));
		}
		break;
		...
		case GAP_EVT_SMP_SECURITY_PROCESS_DONE:
		{
			...
			tlkapi_send_string_data(APP_SMP_LOG_EN, "[APP][SMP] Security process done event", &p->connHandle, sizeof(gap_smp_securityProcessDoneEvt_t));
