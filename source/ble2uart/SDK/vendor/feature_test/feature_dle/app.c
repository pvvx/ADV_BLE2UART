...
{
	u16 connHandle;
	for(u8 i= 0; i < MASTER_ACL_PERIPHR_MAX_NUM; i++){
		...
		if(conn_dev_list[i].conn_state==1) //connect state
		{
			...
			if(tst_flg && dle_started_flg[i] && mtuExchange_started_flg[i] && clock_time_exceed(mtu_tick[i], 1000*1000))
			{
				...
				{
					u16 dataLen = min(mtuExchange_started_flg[i]-3, ATT_MTU_MASTER_RX_MAX_SIZE-3);
					if(BLE_SUCCESS == blc_gatt_pushWriteCommand (connHandle, SPP_CLIENT_TO_SERVER_DP_H, &app_test_data[i][0], dataLen))
					{
				...
				{
					u16 dataLen = min(mtuExchange_started_flg[i]-3, ATT_MTU_MASTER_RX_MAX_SIZE-3);
					if(BLE_SUCCESS == blc_gatt_pushHandleValueNotify (connHandle, SPP_SERVER_TO_CLIENT_DP_H, &app_test_data[i][0], dataLen))
					{
						mtu_tick[i] = clock_time();
						app_test_data[i][0] ++;
					}
	...
	if (h &HCI_FLAG_EVENT_BT_STD)		//Controller HCI event
	{
		...
		else if(evtCode == HCI_EVT_LE_META)  //LE Event
		{
			...
			else if (subEvt_code == HCI_SUB_EVT_LE_CONNECTION_UPDATE_COMPLETE)	// connection update
			{
				...
				tlkapi_send_string_u32s(APP_CONTR_EVENT_LOG_EN, "[APP][CMD] DLE exchange", pDleParam->connHandle, pDleParam->maxTxOct, pDleParam->maxRxOct, 0);
			}
		}
	}
...
int app_host_event_callback(u32 h, u8 *para, int n)
{
	...
	switch(event)
	{
		...
		case GAP_EVT_ATT_EXCHANGE_MTU:
		{
			...
			mtuExchange_started_flg[conn_idx] = pEvt->effective_MTU;//1;   //set MTU size exchange flag here

			tlkapi_send_string_data(APP_HOST_EVENT_LOG_EN, "[APP][EVT]Effective_MTU = ", &pEvt->effective_MTU, 2);
		}
		break;
