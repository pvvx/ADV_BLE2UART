...
int app_le_ext_adv_report_event_handle(u8 *p, int evt_data_len)
{
	...
	if(pExtAdvRpt->num_reports != 1){
		...
		//tlkapi_send_string_data(APP_LOG_EN, "[APP][EVT]data", pExtAdvRpt, evt_data_len);
	}
	...
	for(int i=0; i<pExtAdvRpt->num_reports ; i++)
	{
		...
		//TODO: add a function process data combine
				...
				if(pExtAdvInfo->event_type & EXTADV_RPT_EVT_MASK_LEGACY)
				{
					...
					if(!(aux_adv_cnt & 0x1F)){  //every 32
						tlkapi_send_string_data(APP_LOG_EN, "[APP][EVT]aux adv rpt", pExtAdvInfo, EXTADV_INFO_LENGTH + pExtAdvInfo->data_length);
					}
					aux_adv_cnt = 0;
					//aux_adv_cnt ++;
				}
		...
		/* Extended ADV */
			...
			//tlkapi_send_string_data(APP_LOG_EN, "[APP][EVT]ext_conn_adv", pExtAdvInfo, EXTADV_INFO_LENGTH + pExtAdvInfo->data_length);
		}
...
//						DBG_CHN7_TOGGLE;
						if(status != HCI_ERR_CONN_REJ_LIMITED_RESOURCES){
							tlkapi_send_string_data(APP_LOG_EN, "[APP][EVT]ext init fail", &status, 1);
						}
					}
					else{
//						DBG_CHN6_TOGGLE;
