...
int app_le_adv_report_event_handle(u8 *p)
{
	...
	if(central_auto_connect || user_manual_pairing){
        ...
        if(status == BLE_SUCCESS){ //create connection succe ss
...
int app_le_connection_complete_event_handle(u8 *p)
{
	...
	if(pConnEvt->status == BLE_SUCCESS){
		...
		#if (UI_LED_ENABLE)  //led show connection state
		...
		if( pConnEvt->role == ACL_ROLE_CENTRAL ) // master role, process SMP and SDP if necessary
		{
				...
				if( dev_char_info_search_peer_att_handle_by_peer_mac(pConnEvt->peerAddrType, pConnEvt->peerAddr, pdev_att) ){
					cur_sdp_device.char_handle[0] = pdev_att->char_handle[0];           //MIC
...
{
	event_disconnection_t	*pDisConn = (event_disconnection_t *)p;
...
#if (UI_LED_ENABLE)
	if(acl_conn_central_num == 0){ //LED off when no connection
...
_attribute_no_inline_ void main_loop(void)
{
	...
	if(clock_time_exceed(print_tick, 2000000)){
		print_tick = clock_time();
		//tlkapi_send_string_data(APP_LOG_EN, "[APP][LOOP]", &print_cnt, 4);
		print_cnt ++;
#if 0
		DBG_CHN0_TOGGLE;
		DBG_CHN1_TOGGLE;
